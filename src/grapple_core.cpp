...
Rcpp::List grapple_stat_multi_cpp(const arma::cube& RxyList,
  ...
  arma::cube bias_thread(p, p, std::max(n_threads, 1), arma::fill::zeros);
...
// hi = Mi[1,1]*theta - Mi[1,2]  (using 0-based index: (0,0) and (0,1))
double hi = Mi(0,0) * theta - Mi(0,1);
var_cor(i) = hi;

// bias_cor += Mi[1,1] / var_i
bias_thread(tid) += Mi(0,0) / v_i;
}
