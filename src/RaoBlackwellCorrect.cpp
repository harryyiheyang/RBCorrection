...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < m; ++i) {
    ...
    if (!accepted.empty()) {
      ...
      CovList[i] = arma::cov(acc_mat, 0);  // normalize by N
    } else {
      BETA_RB.row(i).fill(NA_REAL);
      SE_RB.row(i).fill(NA_REAL);
      CovList[i] = arma::mat(p1, p1, arma::fill::none);
      CovList[i].fill(NA_REAL);
    }
  }
